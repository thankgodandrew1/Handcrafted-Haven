...
async function authenticate({ email, password }: { email: string; password: string }): Promise<{ token: string; } & UserAttributes> {
  ...
  const token: string = jwt.sign({ sub: user.id }, serverRuntimeConfig.secret, { expiresIn: '7d' });
...
async function update(id: string, params: { email: string; password: string }): Promise<void> {
  const user: UserDocument | null = await User.findById(id);
