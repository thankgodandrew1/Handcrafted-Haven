...
const ProductListings: React.FC<ProductListingsProps> = ({ productlistings }) => {
  ...
  <section className="py-12 px-6 bg-gradient-to-r from-muted to-background m-4 rounded-lg mt-0">
    ...
    <div className="grid grid-cols-6 md:grid-cols-5 gap-6">
      <div className="row-start-1 row-end-7">
        ...
        />
        <button type="submit">
          <i className="fa fa-search float-left border-l-0 text-white 
          bg-secondary w-16 p-3 rounded-r-xl -mt-10 mb-6 ml-32"></i>
        </button>
      ...
      {currentProducts.map((product) => (
        <div
          ...
          className="transition-transform duration-300
            transform-gpu hover:scale-105 bg-background rounded-lg shadow-lg p-4"
        >
          <Image
            width={200}
            height={200}
          ...
          />
          <h3 className="text-lg font-semibold font-heading mb-2">{product.name}</h3>
          <p className="text-text font-body">{product.description}</p>
          <p className="text-text text-lg font-bold font-body mt-5">Price: {product.price}</p>
          <button
      ...
      {shouldDisplayPagination && (
        <div className="flex items-center justify-center p-3">
          {[...Array(Math.ceil(displayedProducts.length / productsPerPage))].map(
            (_, index) => (
              <button
              className={`p-2 bg-muted hover:bg-link transition duration-30
              text-white font-bold m-2 rounded-lg ${currentPage === index + 1 ? 'bg-link' : ''}`}
                key={index}
